...
function applyGlobalStyles() {
    // Проверяем, не добавлены ли уже глобальные стили
    if (document.getElementById('editor-global-styles')) return

    // Глобальные стили
    const globalLink = document.createElement('link')
    globalLink.id = 'editor-global-styles'
    globalLink.rel = 'stylesheet'
    globalLink.href = '/editor/ui/styles/global.css'
    document.head.appendChild(globalLink)

    // Стили компонентов
    const componentsLink = document.createElement('link')
    componentsLink.id = 'editor-components-styles'
    componentsLink.rel = 'stylesheet'
    componentsLink.href = '/editor/ui/styles/components.css'
    document.head.appendChild(componentsLink)

    console.log('🎨 Глобальные стили загружены')
}
...
export function cleanupUI() {
    ...
    // Удаляем стили
    const styleTags = ['editor-global-styles', 'editor-components-styles']
    styleTags.forEach(id => {
